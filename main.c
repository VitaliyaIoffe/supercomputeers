...
// FILL MATRIX B
    ...
    int k = 0;
    for (int out_ind = 0; out_ind < 2; ++out_ind) {
        for (int i = 0; i < M + 1; ++i) {
            for (int j = 0; j < N + 1; ++j) {
                w[out_ind][i][j] = 0;
            }
...
//        printf("LOG: Start iteration #%d\n", k + 1);
        ...
        pertrubed_f = apply_operator(w[0], pertrubed_f, x, y, q);

        r = matrix_difference(pertrubed_f, b, c, M + 1, N + 1);
        ...
        current_norm = norm(matrix_difference(w[1], w[0], c, M + 1, N + 1));
